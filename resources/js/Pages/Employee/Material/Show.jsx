...
export default function EmployeeMatShow() {
  ...
  const accept = (e) => {
    ...
    acceptForm.post(route('employee.materiel.accept', req.id), {
      onSuccess: () => {
        Swal.fire({ icon: 'success', title: 'Demande acceptée', timer: 1200, showConfirmButton: false }).then(() =>
          router.visit(route('employee.materiel.index'))
        );
      },
    });
  };
  ...
  const reject = (e) => {
    ...
    rejectForm.post(route('employee.materiel.reject', req.id), {
      onSuccess: () => {
        Swal.fire({ icon: 'success', title: 'Demande rejetée', timer: 1200, showConfirmButton: false }).then(() =>
          router.visit(route('employee.materiel.index'))
        );
      },
    });
    ...
    <DashboardLayout title="Détail - Ressources matériel">
      <div className="mb-4">
        <div className="text-sm text-gray-500">
          ...
          Matricule : <span className="font-medium">{req.matricule || 'N/A'}</span>
        </div>
        <div className="text-xs text-gray-500">Créé le {new Date(req.created_at).toLocaleString('fr-FR')}</div>
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        <FileRow label="Contrôle réglementaire" href={route('employee.materiel.download', [req.id, 'controle_reglementaire'])} />
        <FileRow label="Assurance" href={route('employee.materiel.download', [req.id, 'assurance'])} />
        <FileRow label="Habilitation du conducteur" href={route('employee.materiel.download', [req.id, 'habilitation_conducteur'])} />
        <FileRow label="Rapports de chantier et conformité" href={route('employee.materiel.download', [req.id, 'rapports_conformite'])} />
      </div>
      ...
      <div className="mt-4">
        <Link href={route('employee.materiel.index')} className="text-sm text-gray-600 hover:underline">
          ← Retour
        </Link>
      </div>
    </DashboardLayout>
