...
const contractorProject = auth?.contractor?.project?.name || "";
  ...
  const [comportements, setComportements] = useState([
    { type: "", description: "", photo: null },
  ]);
  ...
  const dangerousConditionsList = [
    ...
    "Qualifications, habilitation et formation",
    "Inappropriés aux risques",
    "Housekeeping insatisfaisant",
    "Mauvaise ergonomie",
    "Mauvais état de conformité",
    "Non portés correctement",
    "Gestion de déchets insatisfaisante",
    "Problème d’accessibilité",
    "Inappropriés aux tâches",
    "Inadéquats aux tâches",
    "Non appliqués ou appliqués incorrectement",
    "En mauvais état",
    "Présence de fuites et émanations",
    "Non utilisés correctement",
    "Personnel non formé",
  ...
  const validateForm = () => {
    ...
    if (!header.personnesObservees[0])
      newErrors.personnesObservees = "Au moins une personne observée est requise.";
    header.personnesObservees.forEach((p, i) => {
      if (p && !onlyLetters(p))
        newErrors[`personnesObservees_${i}`] =
          "Seules les lettres et espaces sont autorisés.";
    });

    if (!header.entrepriseObservee[0])
      newErrors.entrepriseObservee = "Au moins une entreprise observée est requise.";
    header.entrepriseObservee.forEach((e, i) => {
      if (e && !onlyLetters(e))
        newErrors[`entrepriseObservee_${i}`] =
          "Seules les lettres et espaces sont autorisés.";
    });

    pratiques.forEach((p, i) => {
      if (p.text && !p.photo)
        newErrors[`pratique_${i}`] =
          "L’image est requise pour la bonne pratique.";
    });
    comportements.forEach((c, i) => {
      if ((c.type || c.description) && !c.photo)
        newErrors[`comportement_${i}`] =
          "L’image est requise pour le comportement.";
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateForm()) {
      ...
      await Swal.fire({
        ...
        text: "Merci de corriger les erreurs surlignées.",
      });
      return;
    }
    ...
    router.post("/contractant/vods/store", formData, {
      forceFormData: true,
      headers: { "Content-Type": "multipart/form-data" },
      preserveScroll: false,

      onFinish: () => {
        if (opened && Swal.isVisible()) Swal.close();
      },
      ...
      onError: async (serverErrors) => {
        setTopAlert(
          "Des erreurs de validation sont survenues. Merci de corriger les champs en rouge."
        );
        ...
        await Swal.fire({
          ...
          title: "Erreur",
          text:
            serverErrors && Object.keys(serverErrors).length
              ? "Vérifiez les champs indiqués en rouge."
              : "La requête a échoué (session expirée ou erreur serveur).",
        });
      },
    });
  };
    ...
    >
      ...
      {topAlert && (
        <div
          ...
          role="alert"
          tabIndex={-1}
          className="p-3 sm:p-4 rounded border border-red-400 bg-red-50 text-red-700"
        >
          {topAlert}
        </div>
      )}
      {flash.success && (
        <div
          role="status"
          className="p-3 sm:p-4 bg-green-100 border border-green-400 text-green-700 rounded"
        >
          {flash.success}
        </div>
      )}
...
/>
          ...
          <div className="md:col-span-3 space-y-2">
            ...
            <div className="space-y-2">
              ...
              {errors.personnesObservees && (
                <p className="text-red-500 text-sm">{errors.personnesObservees}</p>
              )}
              ...
              <button
                ...
                onClick={() =>
                  setHeader({
                    ...header,
                    personnesObservees: [...header.personnesObservees, ""],
                  })
                }
          ...
          <div className="md:col-span-3 space-y-2">
            ...
            <div className="space-y-2">
              ...
              {errors.entrepriseObservee && (
                <p className="text-red-500 text-sm">{errors.entrepriseObservee}</p>
              )}
              ...
              <button
                ...
                onClick={() =>
                  setHeader({
                    ...header,
                    entrepriseObservee: [...header.entrepriseObservee, ""],
                  })
                }
                className="text-blue-600 hover:underline text-sm"
              >
        ...
        <SectionCard title="Bonnes pratiques" defaultOpen={!isMobile}>
          {pratiques.map((item, i) => (
            <div
              key={i}
              className="space-y-2 sm:space-y-0 sm:flex sm:items-start sm:gap-3 mb-3"
            >
              <input
                type="text"
              ...
              <div className="flex items-center gap-2">
                <ImageInput
                  file={item.photo}
                  onChange={(file) =>
                    handleFileUpload(pratiques, setPratiques, i, file)
                  }
              ...
              {errors[`pratique_${i}`] && (
                <p className="text-red-500 text-sm">{errors[`pratique_${i}`]}</p>
              )}
            </div>
          ))}
          <button
            type="button"
            onClick={() =>
              setPratiques([...pratiques, { text: "", photo: null }])
            }
            className="text-blue-600 hover:underline text-sm"
          >
            + Ajouter
            ...
            <div key={i} className="mb-4 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <ImageInput
                  file={item.photo}
                  onChange={(file) =>
                    handleFileUpload(comportements, setComportements, i, file)
                  }
        ...
        <SectionCard title="Conditions dangereuses" defaultOpen={!isMobile}>
          ...
          {autresConditions.map((val, idx) => (
            <div
              key={idx}
              className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mt-3"
            >
              <input
                type="text"
                ...
                onBlur={(e) => {
                  ...
                  if (name && !conditions[name])
                    setConditions((prev) => ({ ...prev, [name]: true }));
                }}
                className="input w-full sm:flex-1"
              />
              <button
        ...
        <SectionCard title="Actions correctives" defaultOpen={!isMobile}>
          {Object.entries(conditions).filter(([, val]) => val).length === 0 ? (
            <p className="text-sm text-gray-500">
              Sélectionnez des conditions dangereuses pour saisir des actions.
            </p>
          ) : null}
          ...
          {Object.entries(conditions)
            ...
            .map(([key]) => (
              <div
                key={key}
                className="border-l-4 border-green-500 bg-green-50 p-4 rounded mb-4 shadow-sm space-y-2"
              >
                ...
                <input
                  ...
                  placeholder="Action"
                  onChange={(e) =>
                    handleCorrectiveChange(key, "action", e.target.value)
                  }
                  className="input w-full"
                />
                <input
                  ...
                  onChange={(e) =>
                    handleCorrectiveChange(key, "responsable", e.target.value)
                  }
                  className="input w-full"
                />
                <input
                  ...
                  placeholder="Statut"
                  onChange={(e) =>
                    handleCorrectiveChange(key, "statut", e.target.value)
                  }
                  className="input w-full"
                />
                <ImageInput
                  ...
                  onChange={(file) =>
                    handleCorrectiveChange(key, "photo", file)
                  }
      ...
      <div className="md:hidden fixed bottom-4 left-0 right-0 z-40">
        <div className="mx-4 rounded-xl shadow-lg bg-white border p-3">
          <button
            type="submit"
            disabled={!canSubmit}
            className="w-full px-5 py-3 rounded text-white text-base bg-green-600 hover:bg-green-700"
          >
            Soumettre le formulaire
          </button>
        </div>
...
function ImageInput({ file, onChange, ariaLabel }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <label
        htmlFor={inputId}
        className="px-3 py-2 text-xs sm:text-sm rounded border bg-white hover:bg-gray-50 cursor-pointer"
      >
