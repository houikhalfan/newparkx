...
export default function VodsForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    router.post('/vods/store', formData, {
      forceFormData: true,
      headers: { 'Content-Type': 'multipart/form-data' }, // kept as in your working version
      preserveScroll: false,

      // ✅ Always close loader (success, 422, 419/500, or redirect)
      onFinish: () => {
      ...
      onError: async (serverErrors) => {
        ...
        if (!serverErrors || (typeof serverErrors === 'object' && Object.keys(serverErrors).length === 0)) {
          // Non-422 (e.g., 419/401/500) — show generic error
          await Swal.fire({
            icon: 'error',
