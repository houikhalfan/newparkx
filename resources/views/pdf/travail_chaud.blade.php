...
<div class="page">
  ...
  <div class="hdr">
    <div class="logo">
      <img src="{{ public_path('images/logo.png') }}" alt="Logo" style="max-height:50px;">
    </div>
  ...
  <div class="section">
    ...
    <div class="c">
      ...
      <div class="row">
        ...
        <span class="value">De {{ $permis->date_debut }} à {{ $permis->date_fin }}</span>
      </div>
      <div class="row">
        <span class="label">Description de la tâche :</span>
        <span class="value">{{ $permis->description_tache }}</span>
      </div>
      <div class="row">
        <span class="label">Plan sécuritaire réalisé par :</span>
        <span class="value">{{ $permis->plan_securitaire_par }} ({{ $permis->date_plan_securitaire }})</span>
      </div>
      <div class="row">
        <span class="label">Contractant demandeur :</span>
        <span class="value">{{ $permis->contractant_demandeur }}</span>
      </div>
      <div class="row">
        <span class="label">Contractant effectuant :</span>
        <span class="value">{{ $permis->contractant_travail }}</span>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="c">
      ...
      <table class="compact-table">
        <tr>
          @foreach(array_chunk($activites, 5, true) as $chunk)
          <td style="width: 50%;">
            @foreach($chunk as $k => $lbl)
  ...
  <div class="section">
    ...
    <div class="c">
      ...
      <table class="compact-table">
        <tr>
          @foreach(array_chunk($dangers, 5, true) as $chunk)
          <td style="width: 50%;">
            @foreach($chunk as $k => $lbl)
    ...
    <div class="c">
      ...
      <div class="flex2">
        <div>
          ...
          @endphp
          @foreach($protectionPhysique as $k => $lbl)
            <div class="item">{!! $box($isChecked($permis->protection_physique, $k)) !!} {{ $lbl }}</div>
          @endforeach
          @if($isChecked($permis->protection_physique, 'autre') && $permis->protection_physique_autre)
            <div class="item" style="margin-left: 15px;">→ {{ $permis->protection_physique_autre }}</div>
          @endif
        </div>
        ...
        <div>
          ...
          @endphp
          @foreach($protectionRespiratoire as $k => $lbl)
            <div class="item">{!! $box($isChecked($permis->protection_respiratoire, $k)) !!} {{ $lbl }}</div>
          @endforeach
        </div>
      </div>
    </div>
    ...
    <div class="c">
      <div class="flex2">
        <div>
          ...
          @endphp
          @foreach($protectionIncendie as $k => $lbl)
            <div class="item">{!! $box($isChecked($permis->protection_incendie, $k)) !!} {{ $lbl }}</div>
          @endforeach
          @if($isChecked($permis->protection_incendie, 'autre') && $permis->protection_incendie_autre)
            <div class="item" style="margin-left: 15px;">→ {{ $permis->protection_incendie_autre }}</div>
          @endif
        </div>
        ...
        <div>
          ...
          @endphp
          @foreach($equipementInspection as $k => $lbl)
            <div class="item">{!! $box($isChecked($permis->equipement_inspection, $k)) !!} {{ $lbl }}</div>
          @endforeach
        </div>
      </div>
      ...
      <div class="flex2" style="margin-top: 6px;">
        <div>
          ...
          @endphp
          @foreach($permisRequis as $k => $lbl)
            <div class="item">{!! $box($isChecked($permis->permis_requis, $k)) !!} {{ $lbl }}</div>
          @endforeach
        </div>
        ...
        <div>
          ...
          @endphp
          @foreach($surveillance as $k => $lbl)
            <div class="item">{!! $box($isChecked($permis->surveillance_requise, $k)) !!} {{ $lbl }}</div>
          @endforeach
        </div>
      </div>
    </div>
  ...
  <div class="section">
    ...
    <div class="c">
      @if($permis->aucun_commentaire)
        <div class="item">{!! $box(true) !!} Aucun commentaire additionnel ou recommandation</div>
      @else
        <div class="value">{{ $permis->commentaires }}</div>
      @endif
    </div>
  </div>
    ...
    <div class="c">
      <div class="flex2">
        <div>
          ...
          Nom : {{ $permis->resp_construction_nom }}<br>
          Date : {{ $permis->resp_construction_date }}<br>
          @if($permis->resp_construction_signature)
            <img class="sign-img" src="{{ public_path('storage/'.$permis->resp_construction_signature) }}">
          @endif
        </div>
        <div>
          <b>Responsable HSE</b><br>
          Nom : {{ $permis->resp_hse_nom }}<br>
          Date : {{ $permis->resp_hse_date }}<br>
          @if($permis->resp_hse_signature)
            <img class="sign-img" src="{{ public_path('storage/'.$permis->resp_hse_signature) }}">
          @endif
        </div>
      </div>
    </div>
    ...
    <div class="c">
      <div class="flex2">
        <div>
          ...
          @if($permis->cm_parkx_signature)
            <img class="sign-img" src="{{ public_path('storage/'.$permis->cm_parkx_signature) }}">
          @endif
        </div>
        <div>
          ...
          @if($permis->hse_parkx_signature)
            <img class="sign-img" src="{{ public_path('storage/'.$permis->hse_parkx_signature) }}">
          @endif
          @if($permis->hse_parkx_commentaires)
            <div style="margin-top: 3px; font-size: 7pt;">
              <b>Commentaires HSE :</b> {{ $permis->hse_parkx_commentaires }}
            </div>
          @endif
        </div>
      </div>
