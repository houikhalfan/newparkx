...
class Vod extends Model
{
    ...
    // defaults so JSON fields are always arrays
    ...
    protected static function booted(): void
    {
        static::saving(function (Vod $vod) {
            ...
            // Derive danger flags from conditions JSON (truthy values)
            ...
            // Auto-derive risk_breakdown from comportements[].type if empty
            if (empty($vod->risk_breakdown)) {
                $comp = is_array($vod->comportements)
                    ? $vod->comportements
                    : (json_decode($vod->comportements ?? '[]', true) ?: []);
                $labels = [];
                foreach ($comp as $it) {
                    $t = is_array($it) ? ($it['type'] ?? null)
                       : (is_object($it) ? ($it->type ?? null) : $it);
                    if (is_string($t) && ($t = trim($t)) !== '') $labels[] = $t;
                }
                $vod->risk_breakdown = array_values(array_unique($labels));
            }
        });
    }
}
